...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(
        ...
        # host agent，用于管理多个 agent-server（采用agent_card方式管理）
    ...
    def _initialize_host(self):
        # 初始化 agent host，进行创建一个 host agent的执行器，也就是一个具体的 llm_agent
    ...
    async def process_message(self, message: Message):
        # host_manager 中处理消息的方法，也是正式开始处理了
        ...
        # 核心：host_runner 运行器，走这里，也就是具体去执行 host_agent_server了
